...
const deviceColumns = computed(() => {
  const deviceItem = useGetDeviceItem(route.query.id)

  return deviceUseList(route.query.id)
    .filter((use) => use != 'switch')
    .map((item) => {
      const list = deviceItem.columns.filter((columnItem) => columnItem.use == item)
  ...
  <div class="min-h-screen bg-page-gray">
    ...
    <van-radio-group v-model="checked" class="mt-4">
      <van-cell-group inset>
        ...
        <!--设备属性-->
        <div class="pl-2">
          <template v-if="route.query.clasify === '100'">
            <van-cell
