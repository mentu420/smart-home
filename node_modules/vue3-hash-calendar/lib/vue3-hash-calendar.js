(function(global, factory) {
  typeof exports === "object" && typeof module !== "undefined" ? factory(exports, require("vue")) : typeof define === "function" && define.amd ? define(["exports", "vue"], factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, factory(global["vue3-hash-calendar"] = {}, global.Vue));
  ...
  var stdin_default$5 = vue.defineComponent({
    ...
    setup(props, {
      ...
      const weekTitleRef = vue.ref(null);
      const calendarRef = vue.ref(null);
      const calendarItemRef = vue.reactive([]);
      const yearOfToday = vue.ref(new Date().getFullYear());
      const monthOfToday = vue.ref(new Date().getMonth());
      const dayOfToday = vue.ref(new Date().getDate());
      const weekArray = ["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"];
      const calendarWeek = vue.ref(["\u65E5", "\u4E00", "\u4E8C", "\u4E09", "\u56DB", "\u4E94", "\u516D"]);
      const calendarOfMonth = vue.ref([]);
      const calendarOfMonthShow = vue.ref([]);
      const calendarDaysTotalLength = vue.ref(42);
      const lastMonthYear = vue.ref(0);
      const lastMonth = vue.ref(0);
      const nextMonthYear = vue.ref(0);
      const nextMonth = vue.ref(0);
      const checkedDate = vue.ref([]);
      const weekStartIndex = vue.ref(0);
      const translateIndex = vue.ref(0);
      const transitionDuration = vue.ref(0.3);
      const touch = vue.ref({
        x: 0,
        y: 0
      });
      const isTouching = vue.ref(false);
      const calendarGroupHeight = vue.ref(0);
      const calendarWeekTitleHeight = vue.ref(0);
      const calendarItemHeight = vue.ref(0);
      const touchStartPositionX = vue.ref(0);
      const touchStartPositionY = vue.ref(0);
      const calendarY = vue.ref(0);
      const selectedDayIndex = vue.ref(0);
      const lastWeek = vue.ref([]);
      const nextWeek = vue.ref([]);
      const isLastWeekInCurrentMonth = vue.ref(false);
      const isNextWeekInCurrentMonth = vue.ref(false);
      const markDateColorObj = vue.ref({});
      const markDateTypeObj = vue.ref({});
      const currentYearMonth = vue.ref({
      ...
      vue.watch(weekStartIndex, () => {
        calculateCalendarOfThreeMonth();
      });
      vue.watch(() => props.defaultDate, (val) => {
      ...
      vue.watch(currentYearMonth, () => emit("yearMonthChange", currentYearMonth.value));
      vue.watch(isShowWeek, (val) => {
        if (val) {
          vue.nextTick(() => {
            showWeek();
          });
      ...
      vue.watch(() => props.defaultYearMonth, (val) => {
        if (val) {
      ...
      vue.watch(calendarGroupHeight, (val) => {
      ...
      const renderWeek = () => vue.createVNode("div", {
        "class": "calendar_week",
        "ref": weekTitleRef
      }, [calendarWeek.value.map((item) => vue.createVNode("div", {
        "class": "calendar_item",
        "key": item
      }, [vue.createVNode("p", {
        "class": "calendar_day"
      ...
      const renderMonth = () => calendarOfMonthShow.value.map((item, i) => vue.createVNode("li", {
      ...
      }, [item.map((date, j) => vue.createVNode("div", {
      ...
      }, [renderDay(date, i), vue.createVNode("div", {
        "style": {
          background: markDateColor(date, "dot")
        },
        "class": "calendar_dot"
      }, null)]))]));
      const renderCalendarDate = () => vue.createVNode("div", {
        "class": "calendar_body",
        "style": {
          display: props.show ? "block" : "none"
        }
      }, [renderWeek(), vue.createVNode("div", {
  ...
  var stdin_default$4 = vue.defineComponent({
    ...
    setup(props, {
      ...
      const timeContentRef = vue.reactive([]);
      const timeItemRef = vue.reactive([]);
      const checkedDate = vue.ref({
        hours: new Date().getHours(),
        minutes: new Date().getMinutes()
      });
      const timeHeight = vue.ref(0);
      const timeArray = vue.ref([]);
      const timeStartY = vue.ref(0);
      const timeStartUp = vue.ref(0);
      const initTimeArray = () => {
        const hours = [];
        timeArray.value = [];
        for (let i = 0; i < 24; i++) {
      ...
      vue.watch(() => props.defaultTime, (val) => {
      ...
      vue.watch(() => props.minuteStep, (val) => {
        if (val <= 0 || val >= 60) {
      ...
      const renderTimeItem = (time, index) => time.map((item, j) => vue.createVNode("div", {
        "ref": (ref2) => {
      ...
      const renderCalendarTime = () => vue.createVNode("div", {
        "class": "time_body",
        "style": {
        ...
        "class": "time_group"
      }, [timeArray.value.map((item, index) => vue.createVNode("div", {
        "class": "time_content",
        "ref": (ref2) => {
  ...
  var stdin_default$3 = vue.defineComponent({
    ...
    setup(props, {
      ...
      const renderContainer = () => vue.createVNode("ul", {
  ...
  var stdin_default$2 = vue.defineComponent({
    ...
    setup(props, {
      ...
      const yearRange = vue.ref(10);
      const disabledScrollDirec = vue.ref(false);
      const yearMonthShow = vue.ref([]);
      const lang = props.lang.toUpperCase();
      language.value = stdin_default$6[lang];
      const itemHeight = vue.computed(() => props.calendarContentHeight / 4);
      const initYear = (year) => {
        const yearArr = [];
      ...
      const renderYearMonthItem = (arr) => arr.map((item, index) => vue.createVNode("div", {
  ...
  var stdin_default$1 = vue.defineComponent({
    ...
    setup(props, {
      ...
      const isShowCalendar = vue.ref(true);
      const isShowWeek = vue.ref(props.showWeekView);
      const calendarBodyHeight = vue.ref(0);
      const firstTimes = vue.ref(true);
      const currentYearMonth = vue.ref({
        year: new Date().getFullYear(),
        month: new Date().getMonth()
      });
      ...
      const currDateTime = vue.ref(getInitialDateTime());
      const checkedDate = vue.ref([transDateToHourMinute(new Date())]);
      const yearMonthType = vue.ref("date");
      const isShowDatetimePicker = vue.computed({
        get() {
      ...
      const isShowArrowImg = vue.computed(() => props.showArrow && props.model === "inline");
      const calendarContentHeight = vue.computed(() => {
      ...
      vue.watch(() => props.showWeekView, (val) => {
        isShowWeek.value = val;
      });
      vue.watch(() => [props.selectType, props.minDate, props.maxDate], () => {
        if (props.selectType === "single") {
      ...
      const renderAction = () => {
        ...
        if (props.showAction) {
          ...
          }, [slots.action ? slots.action() : vue.createVNode(vue.Fragment, null, [vue.createVNode("div", {
            "class": "calendar_title_date"
          }, [props.pickerType !== "time" ? vue.createVNode("span", {
            "class": `calendar_title_date_year ${isShowCalendar.value ? "calendar_title_date_active" : ""}`,
            "onClick": showCalendar
          }, [dateText]) : null, props.pickerType !== "date" && props.selectType === "single" ? vue.createVNode("span", {
      ...
      const renderCalendar = () => vue.createVNode(stdin_default$5, vue.mergeProps({
        "ref": calendarRef,
      ...
      const renderTimePicker = () => {
        if (props.pickerType === "datetime" || props.pickerType === "time") {
          return vue.createVNode(stdin_default$4, vue.mergeProps({
            "show": !isShowCalendar.value,
      ...
      const renderYearMonthPicker = () => vue.createVNode(stdin_default$2, vue.mergeProps({
      ...
      const renderCtrlImg = () => {
        let confirmEle = vue.createVNode("img", {
      ...
      const renderHashCalendar = () => {
        if (isShowDatetimePicker.value || props.model === "inline") {
          return vue.createVNode("div", {
            ...
            "onClick": close
          }, [renderAction(), vue.createVNode("div", {
